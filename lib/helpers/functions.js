...
function loopThrough(previous, current, runEveryTicks, valueKey) {
    ...
    if (Array.isArray(current)) {
        ...
        for (const key of Object.keys(current)) {
            if (!blackListAverageKeys.includes(key)) {
                if (previous === undefined) previous = {};
        ...
        if (!blackListAverageKeys.includes(valueKey)) current = exponentialMovingAverage(current, previous, Math.round(1500 / runEveryTicks));
    }
