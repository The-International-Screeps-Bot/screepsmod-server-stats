...
function loopThrough(previous, current, runEveryTicks, valueKey) {
    const typeofCurrent = typeof current
    if (Array.isArray(current)) {
        for (const element of current) {
            if (!blackListAverageKeys.find(element)) {
                if (previous === undefined) previous = [];
                const objects = loopThrough(previous[element], current[element], runEveryTicks, element);
        ...
        for (const key of Object.keys(current)) {
            if (!blackListAverageKeys.find(key) && Object.hasOwnProperty.call(current, key)) {
                if (previous === undefined) previous = {};
                const objects = loopThrough(previous[key], current[key], runEveryTicks, key);
        ...
        if (!blackListAverageKeys.find(valueKey)) current = exponentialMovingAverage(current, previous, Math.round(1500 / runEveryTicks));
    }
...
const defaultConfig = { rawExports: false, usersStats: 2, roomsObjectsStats: 2, roomsIntentsStats: 2, runEveryTicks: 50 }
function getConfig() {
    try {
        let filename
...
async function getUsers(query) {
    ...
    log(`Found ${users.length} users in ${Math.round(getMicroTimestamp() - start)} microseconds`, 'logs/engine.log')
    return users
}
async function getRoomsObjects(query) {
    const start = getMicroTimestamp()
    const roomsObjects = await query
    log(`Found ${roomsObjects.length} rooms objects in ${Math.round(getMicroTimestamp() - start)} microseconds`, 'logs/engine.log')
    return roomsObjects
}
async function getRoomsIntents(query) {
    const start = getMicroTimestamp()
    const roomsIntents = await query
    log(`Found ${roomsIntents.length} rooms intents in ${Math.round(getMicroTimestamp() - start)} microseconds`, 'logs/engine.log')
    return roomsIntents
}
